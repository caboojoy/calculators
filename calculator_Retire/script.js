  ...
  /* ═══════════════════════════════════════════════════════════
    ...
    function calcServiceInfo(startStr, endStr) {
      ...
      // 퇴사일을 +1일 보정 — 입사일과 퇴사일을 모두 재직일로 포함(inclusive)하는
      // 실무 관행에 맞춰 모든 계산(일수·연수·readable)에 일관 적용
      const eRaw = parseLocalDate(endStr);
    ...
    function validate(s, e, salary, days) {
      ...
      if (s && e) {
        ...
        if (ed < sd) {
          ...
          const diffDays = calcServiceInfo(s, e).totalDays;
          if (diffDays < 365) {
